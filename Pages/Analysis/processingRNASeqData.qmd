...
::: callout-important
**These analyses can't be run on the cluster head node. You will either need to generate a script to submit to the LSF scheduler, or request an interactive session.** **Quality control, mapping, and tag directories**
:::
...
**These tools: depend on file names in the lab convention (\*R1.fastq.gz); require input of only one fastq.gz file per sample per read direction; generate tag directories assuming a stranded library chemistry (e.g. dUTP RNA-seq protocol).**
:::
