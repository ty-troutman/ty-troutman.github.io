...
We have access to a Linux [computational cluster](https://hpc.research.cchmc.org/accessing-the-cluster) through the [Data Management and Analysis Collaborative (DMAC)](https://centerlink.cchmc.org/research/research---tabs/dmac/grant-boilerplate). To request access to the cluster, please use the linked [web form](https://hpc.research.cchmc.org/node/36), or send an email to Cluster Support: help-cluster\@bmi.cchmc.org. After your access is enabled, you should be able to login to the cluster with your CCHMC employee network credentials. 

Up to date policies are described [here](https://hpc.research.cchmc.org/node/13). In brief, all users have a default disk (home directory) quota of 100 GB and a job walltime quota of 10000 hours per quarter, which is free. The quota of your home directory is fixed, and cannot be changed. To increase the walltime, please send an email to [Cluster Support](mailto:help-cluster@bmi.cchmc.org).

Your [home directory](https://en.wikipedia.org/wiki/home_directory) is accessible at the path `/users/YOURUSERID` and has a default 100 GB quota. You also have access to a "[scratch](https://hpc.research.cchmc.org/node/14)" volume at `/scratch/YOURUSERID`, where there is a 100 GB quota with a burst of up to 5 TB for up to 7 days. Anything there will be removed after 60 days. Users typically do not have (read/write) access to other users' home directories. Thus, [`/data/hpc-troutman/`](server_lab_drives.qmd) is the place to store any data or analyses (*not personal files*) that anyone else in the lab might ever need to work with.

# Accessing the HPC cluster

Connect to the CCHMC cluster by following the [BMI instructions]https://hpc.research.cchmc.org/accessing-the-cluster), or [here](https://hpc.research.cchmc.org/node/26), or follow the examples below.
...
-   Alternatively, connect through the [OnDemand Portal](https://ondemand.research.cchmc.org/pun/sys/dashboard). Detailed instructions are [here](https://cchmc-my.sharepoint.com/personal/roberto_perea_cchmc_org/_layouts/15/onedrive.aspx?id=%2Fpersonal%2Froberto%5Fperea%5Fcchmc%5Forg%2FDocuments%2FShared%20Documents%2FOndemand%5FUserGuide%2Epdf&parent=%2Fpersonal%2Froberto%5Fperea%5Fcchmc%5Forg%2FDocuments%2FShared%20Documents&ga=1)
...
The cluster batch system is currently managed by LSF resource manager and scheduler. You can find some examples of submitting jobs using LSF [here](https://hpc.research.cchmc.org/node/1). Please read the job scheduling policies [page](https://hpc.research.cchmc.org/node/13) for recent policy changes.
...
::: callout-note
We have wrapper scripts to automate assembly of .bat files and submission to the LSF scheduler. More information is available [here](analysis_rna_seq.qmd).
:::
