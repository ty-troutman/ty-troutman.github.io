...
opt_parser = OptionParser(
  "\n\tRscript Wrapper_STAR_PairedEnd.R -d path/to/fastq/data [options]",
  option_list = option_list,
  prog = "Wrapper_STAR_PairedEnd.R", 
  ...
  \n\tCommands are appropriate for stranded RNA-seq data (resulting from the standard Troutman lab protocol) and sequenced with paired-end chemistry! 
  ...
  \n\tFor example:
  \t\tRscript /data/hpc-troutman/software/scripts/Wrapper_STAR_PairedEnd.R -d /data/troutman-nobackup/username/directory/ -g mm10 -w 2:00 -c 24 -m 40000 -o BAM SortedByCoordinate\n\n"
)
...
fastqData <- 
  tibble(
    read1 = list.files(file.path(dirPath), pattern = "_R1.fastq.gz"),
    read2 = list.files(file.path(dirPath), pattern = "_R2.fastq.gz"),
  ) %>%
  mutate(outPrefix = str_replace(read1, "_R1.fastq.gz", "")) %>%
  select(outPrefix, read1, read2)

# function to print .bat script
fn1 <- function(read1, read2, outPrefix){
  print(str_c(
    "#BSUB -W ", wall,
    "\n#BSUB -n ", cores,
    "\n#BSUB -M ", memory,
    ...
    --readFilesIn ", file.path(dirPath, read1), " ", file.path(dirPath, read2), " \\
    ...
    -genome ", genome, " -checkGC -format sam -sspe -flip\n",
